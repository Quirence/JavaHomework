...
public class NegativeTextAnalyzer extends KeywordAnalyzer {
	...
	@Override public Label processText(String text) {
		for (String bannedWord : this.getKeywords()) {
			if ((text.indexOf(bannedWord) == 0
				|| text.indexOf(bannedWord) == text.length() - bannedWord.length())
				&& text.indexOf(' ' + bannedWord) != -1) {
